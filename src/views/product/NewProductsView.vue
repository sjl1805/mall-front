...
const loadData = async () => {
  loading.value = true
  try {
    const newProducts = await productStore.fetchNewProducts(pageSize.value)
    products.value = newProducts
  } catch (error) {
    console.error('获取新品上市商品失败', error)
...
<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <p class="page-desc">发现最新上架的精选商品</p>
    </div>
    ...
    <div v-else>
      <div v-if="products.length === 0" class="empty-container">
        <el-empty description="暂无新品上市，请稍后再来" />
      </div>
